...
QBCore.Functions.CreateCallback("qb-multicharacter:server:SetupNewCharacter", function(source, cb)
    local license = QBCore.Functions.GetIdentifier(source, 'license')
    local plyChars = {}
    local Default = 0x0D7114C9

    MySQL.Async.fetchAll('SELECT * FROM players WHERE license = ?', {license}, function(data)
        for k, v in pairs(data) do
            local result = MySQL.Sync.fetchAll('SELECT * FROM playerskins WHERE citizenid = ? AND active = ?', {v.citizenid, 1})

            plyChars[k] = {
                (result[1] and result[1].model) or Default,
                result[1] and result[1].skin,
                v.cid,
                json.decode(v.charinfo),
                v
            }
